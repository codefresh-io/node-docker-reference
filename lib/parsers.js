...
exports.parseQualifiedNameV2 = (name) => {
    const _referenceRegexp = new RE2(referenceRegexp)
    const matches = _referenceRegexp.exec(name);
    ...
    if (!matches) {
        ...
        if (_referenceRegexp.test(name.toLowerCase())) {
            throw new NameContainsUppercaseError();
        }
